...
const UpdateWorkoutPlan = () => {
  ...
  const { id } = useParams(); // Assuming you pass workout id via URL
  ...
  useEffect(() => {
    const fetchWorkoutPlan = async () => {
      try {
        ...
        setMessage('Error loading workout plan');
      }
    };
  ...
  return (
    <div style={containerStyle}>
      <h1 style={headingStyle}>Update Workout Plan</h1>
      <form onSubmit={handleSubmit}>
        ...
        <div>
          <button type="submit" style={buttonStyle}>Update Workout Plan</button>
        </div>
      </form>
